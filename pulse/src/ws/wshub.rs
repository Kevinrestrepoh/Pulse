...
pub type SubscriberTx = mpsc::Sender<Arc<Event>>;
...
impl WsHub {
    ...
    pub async fn subscribe(&self, topic: String, tx: SubscriberTx) -> Result<()> {
        if topic.is_empty() {
            return Err(PulseError::InvalidTopic(
        ...
        if topic.len() > 255 {
            return Err(PulseError::InvalidTopic(
                "Topic too long (max 255 chars)".to_string(),
            ));
        }
            ...
            for sub in subscribers {
                match priority {
                    crate::models::event::Priority::Critical => {
                        sub.send(event.clone()).await.map_err(|e| {
                            PulseError::Channel(format!("Critical event send failed: {}", e))
                        })?;
